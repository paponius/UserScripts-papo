...
// infopanel
// options
// container: not visible, just holds CPControls together
// <custom>: any name, which is not one of the above is accepted and panel without any CSS is created.
//           It will have a classname "cp-<custom>", to allow external CSS customizing.
//
//
// Usage of CPPanel:
...
static css = {
	...
	`,
	hookedOnBody: `:where(.cp-hooked-on-body):not(specificityone) {
			all: revert;
			*:not(path) {
				all: revert;
				color: white;
		...
		.cp-hooked-on-body.cp-hooked-on-body {
			position: fixed;
			right: 5px;
			z-index: 9999999999;
		}`
 ...
 * @param {Object|null|true|String}    options/openButton  If a button is used to open this panel, optionally the CSS class name for it.
 * @returns this
 */
constructor(elParent = null, typePanel = 'options', options = undefined) {
	var openButton;
	if (options instanceof Object) {
		openButton = options.openButton;
		if (options.pid) {
			this.pid = options.pid;
			CPPanel.itemsByPid[options.pid] = this; }
	} else { openButton = options; }
	...
	/// close button
	if (['options', 'infopanel'].includes(typePanel)) {
...
addItem(payload, type, valueDefault, callBack, cid) {
	this.addControl(payload, type, valueDefault, callBack, cid);
	console.warn('change this method to addControl()');
}
addControl(payload, type, valueDefault, callBack, cid) {
	//           (payload, type, panelOrElParent, valueDefault, callBack, cid)
	new CPControl(payload, type, this, valueDefault, callBack, cid);
...
//  todo 2602 and this method should be in CPControl, 2602b: it's ok here

// cid: optional, if undefined, all values are updated
updatePanelValues(cid) {
	this.controls.some(item => {
		if (!cid || item.cid === cid) {
			...
			item.value = item.value; // eslint-disable-line no-self-assign
			if (cid) { return true; } // to end some()
		}
	});
}
...
static create(elParent = null, typePanel = 'options', options = undefined) {
	return new this(elParent, typePanel, options);
}
