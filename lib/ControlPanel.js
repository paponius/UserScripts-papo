 ...
 */
constructor(type, strMessage, defaultValue, callBack, strValueName, typePanel, injectionPoint, idPanel = 0) {
	...
	if (!ControlPanel.panels[idPanel]) {
		ControlPanel.panels[idPanel] = this.panel = ControlPanel.createPanel(idPanel, typePanel, injectionPoint);
	}
	switch (type) {
	 	case 'checkbox':
...
static injectTop(element) {
		...
		switch (document.body.tagName.toUpperCase()) {
			...
			case 'FRAMESET':
				...
				break;
			default:
				console.error('[ControlPanel] unknown page structure');
				debugger;  // jshint ignore:line
				break;
		}
}

static createPanel(id, typePanel = 'Options', injectionPoint = null) {
	var elPanel = document.createElement('DIV');
	this.addCSS(this.cssPanel[typePanel]);
	elPanel.classList.add('cp-options');
	elPanel.classList.add('cp-hidden');

	switch (true) {
		case injectionPoint === null:
			this.injectTop(elPanel);
			break;
		case injectionPoint instanceof HTMLElement:
			injectionPoint.appendChild(elPanel);
			break;
		case injectionPoint instanceof Function:
			injectionPoint(elPanel);
			break;
		default:
			throw '[ControlPanel] wrong injectionPoint';
	}
